 ...
 **/
template <typename T> void print_vector(std::vector<T> v, std::string s = "") {
  std::cout << s;
  for (T element : v)
    std::cout << " " << element;
  std::cout << std::endl;
...
int main() {
  ...
  for (std::vector<int>::iterator it4 = intvector4.begin();
       it4 != intvector4.end(); it4++) {
    std::cout << *it4 << " ";
  }
  std::cout << std::endl;
  ...
  for (int i = 0; i < 5; i++)
    std::cout << "\t Index" << i << ": " << intvector3.at(i) << " or "
              << intvector3[i] << std::endl;

  // differences when accessing outside of range/bounds
  try {
    int a1 = intvector3[5];
    std::cout
        << "out of bounds/range for access vector[index] not detected; val="
        << a1 << std::endl;
  } catch (std::out_of_range &) {
    std::cout << "out of bounds/range for access vector[index] detected"
              << std::endl;
  }
  try {
    int a2 = intvector3.at(5);
    std::cout
        << "out of bounds/range for access vector.at(index) not detected; val="
        << a2 << std::endl;
  } catch (std::out_of_range &) {
    std::cout << "out of bounds/range for access vector.at(index) detected"
              << std::endl;
  }
  ...
  std::cout << "\tvec.front()    " << intvector3.front()
            << " // first element of the vector" << std::endl;
  std::cout << "\tvec.back()     " << intvector3.back()
            << " // last element of the vector" << std::endl;
  std::cout << "\tvec.empty()    " << intvector3.empty()
            << " // is vector empty?" << std::endl;
  std::cout << "\tvec.size()     " << intvector3.size()
            << " // number of elements present in vector" << std::endl;
  std::cout << "\tvec.capacity() " << intvector3.capacity()
            << " // overall size of vector" << std::endl;

  std::cout << "Using these functions on an empty vector: " << std::endl;
  std::vector<int> emptyVec;
  // These two cause segfaults
  // std::cout << "vec.front()    " << emptyVec.front()    << " // first element
  // of the vector" << std::endl; std::cout << "vec.back()     " <<
  // emptyVec.back()     << " // last element of the vector" << std::endl;
  std::cout << "\tvec.empty()    " << emptyVec.empty() << " // is vector empty?"
            << std::endl;
  std::cout << "\tvec.size()     " << emptyVec.size()
            << " // number of elements present in vector" << std::endl;
  std::cout << "\tvec.capacity() " << emptyVec.capacity()
            << " // overall size of vector" << std::endl;
  ...
  std::cout << "*iter=" << *iter << "; iter[0]=" << iter[0]
            << "; iter[3]=" << iter[3] << std::endl;
  std::cout << "int *p = &myVec works too: *p=" << *myPointer
            << "; *(p+3)=" << *(myPointer + 3) << std::endl;

  // Play with addresses
  int i = 0;
  std::cout << "Loop over all elements using iterators:\n";
  for (std::vector<int>::iterator myIter = myVec.begin(); myIter != myVec.end();
       myIter++) {
    std::cout << "\ti=" << i;
    std::cout << "\t*myIter=" << *myIter;
    std::cout << "\t&(*myIter)=" << &(*myIter);
    // std::cout << "\tmyIter=" << myIter; // This doesn't work; iterators don't
