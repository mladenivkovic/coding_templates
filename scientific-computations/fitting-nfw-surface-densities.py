...
def get_profiles(c=c, rho0=rho0, Rvir=Rvir):
    ...
    projected_mass_profile, edges = np.histogram(r, bins = nbins, weights=projected_mass_list, range=(0, Rvir))
    projected_mass_counts, edges = np.histogram(r, bins = nbins, weights=None, range=(0, Rvir))
...
if __name__ == "__main__":
    ...
    for c in cs:
        ...
        ax_dens.semilogy(rd, sigma_nfw(c, rho0, Rvir, rd), label="analytical surface density profile")
        ax_mass.semilogy(rm, projected_mass_nfw(c, rho0, Rvir, rm), label="analytical projected mass profile")
        ...
        # plot fits
        ax_dens.semilogy(rd, sigma_nfw(conlyfit_dens, rho0, Rvir, rd), label="fit to simulated profile, c={0:.3f}".format(conlyfit_dens))
        ax_dens.semilogy(rd, sigma_nfw(cfit_dens, rhofit_dens, Rvir, rd), label="fit to simulated profile, c={0:.3f}, rho0={1:.3f}".format(cfit_dens, rhofit_dens))

        ax_mass.semilogy(rm, projected_mass_nfw(conlyfit_mass, rho0, Rvir, rm), label="fit to simulated profile, c={0:.3f}".format(conlyfit_mass))
        ax_mass.semilogy(rm, projected_mass_nfw(cfit_mass, rhofit_mass, Rvir, rm), label="fit to simulated profile, c={0:.3f}, rho0={1:.3f}".format(cfit_mass, rhofit_mass))
