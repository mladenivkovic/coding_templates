...
int main(void) {
  ...
  // out = fftw_malloc(sizeof(fftw_complex) * N/2+1); // doesn't work for some
  // reason?! not even when type casting to (fftw_complex *) fftw_malloc(..)
  ...
  // my_plan = fftw_plan_dft_c2d_1d(N, out, in, FFTW_ESTIMATE); // backwards
  // transform IMPORTANT: the inverse transform (complex to real) has the
  // side-effect of overwriting its input array
