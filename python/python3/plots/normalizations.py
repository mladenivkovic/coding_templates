...
for i in range(nx):
    for j in range(nx):
        xx = lowlim + (i+0.5)*dx
        yy = lowlim + (j+0.5)*dx
        data_positive_nonzero[j,i] = np.exp(-10*yy**2)*(np.sin(np.pi*4*xx)+2)
        data_no_rules[j,i] = 2*np.sin(np.pi*8*xx**3+8*yy**2) * (np.sin(yy**2) + 2)
...
im = ax.imshow(data_positive_nonzero, norm=clrs.BoundaryNorm(boundaries=bounds, ncolors=256))
divider = make_axes_locatable(ax)
cax = divider.append_axes("right", size="5%", pad=0.05)
fig.colorbar(im, cax=cax)
