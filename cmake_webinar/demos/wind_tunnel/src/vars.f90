module vars
!This module contains the shared variables for the simulation, as well as
!parameters controlling the runs, and some miscellaneous subroutines/functions.

implicit none

    !grid parameters

    !number of cells in x and y direction (global)
    integer :: nx_global = 576
    integer :: ny_global = 576

    !number of cells in x and y direction (local)
    integer :: nx, ny

    !size of windtunnel in metres
    real(8), parameter :: xrange(2) = (/ -2., 2. /)
    real(8), parameter :: yrange(2) = (/ -2., 2. /)

    !grid spacing in "real(8)" units
    real(8) :: dxx 
    real(8) :: dyy

    !grid spacing in computational units
    real(8) :: dx=1.
    real(8) :: dy=1.
    
    ! vorticity parameters

    logical :: vorticity=.true. !set vorticity on/off
    real(8) :: maxvort=5. !maximum allowed vorticity
    real(8) :: r0 = 1E3 !Reynolds number (bigger = less viscous and hence less diffusion)
    real(8) :: vortcoeff = 0.01 !vorticity generated by wings
    real(8) :: vmax = 1.0


    !medium parameters
    real(8) :: p0=1. !pressure (atmospheres)
    real(8) :: rho=1. !density (atomspheres)
    real(8) :: airspeed = 100 !km/h


    !unit conversions
    real(8) :: dtor=3.14159265/180. !deg to rad
    real(8) :: rho_air = 1.3 !kg/m^3
    real(8) :: p_air = 100000. !Pa
    real(8) :: vconv = 1000./3600. !kmph - m/s
    real(8) :: pi=3.141592


    !iteration parameters
    real(8) :: crossing_times=1. !number of crossing times the sim runs for
    real(8) :: dt !timestep
    real(8) :: cfl_r0, cfl_v !timestep CFL conditions for diffusion and advection


    !shape patameters:

    integer :: nose=0 ! (0 centre, 1 nose, 2 centre of mass)

    integer :: shape =2 !1=ellipse, 2=aerofoil

    !position of centre of shape
    real(8) :: x0=0.
    real(8) :: y0=0.

    !angle of attack
    real(8) :: alpha = 0. !degrees


    !ellipse parameters
    real(8) :: a = 0.75 !axis in x direction
    real(8) :: b = 0.75  !axis in y direction


    !aerofoil parameters
    real(8) :: c=2. !chord (length of aerofoil)
    real(8) :: m=0.05 !maximum camber
    real(8) :: t=0.2 !thickness (fraction of length)
    real(8) :: p=0.4 !location of maximum camber (fraction of length along chord)

    ! gpu parameters
    logical :: device = .false.

    !define namelists for reading in config files
    NAMELIST /vortparams/ vorticity,maxvort,r0,vortcoeff
    NAMELIST /mediumparams/  p0,rho,airspeed
    NAMELIST /shapeparams/ shape, alpha, a, b, c, m, t, p,nx_global,ny_global
    NAMELIST /gpuparams/ device






    !global variables
    real(8), allocatable :: x_global(:), y_global(:), u_global(:,:), v_global(:,:)
    real(8), allocatable :: psi_global(:,:), vort_global(:,:)
    integer, allocatable :: mask_global(:,:), boundary_global(:,:)

    !local variables
    real(8), allocatable, dimension(:) :: x, y

    real(8), allocatable, dimension(:,:) :: psi, vort, u, v

    real(8), allocatable, dimension(:,:) :: psidum, dw

    integer, allocatable, dimension(:,:) :: mask, boundary


    

    contains

    subroutine setup_local_arrays()

        allocate(psi(0:nx+1,0:ny+1), vort(0:nx+1,0:ny+1), v(nx,ny), u(nx,ny))
        allocate(psidum(0:nx+1,0:ny+1), dw(0:nx+1,0:ny+1))
        allocate(mask(nx,ny), boundary(nx,ny))
        

    end subroutine


    subroutine setup_global_arrays()

        allocate(x_global(nx_global), y_global(ny_global))
        allocate(psi_global(nx_global,ny_global), vort_global(nx_global,ny_global))
        allocate(v_global(nx_global,ny_global), u_global(nx_global,ny_global))
        allocate(mask_global(nx_global,ny_global), boundary_global(nx_global,ny_global))

    end subroutine


    integer function ellipse(xdum,ydum)
    !given input values of x and y returns 0 or 1 depending upon whether the input
    !coordinartes are within or outwith the allipse

        implicit none

        real(8) :: xdum,ydum
        real(8) :: r
        real(8) :: xdum2, ydum2
        real(8) :: angle

        !rotate the input coordinates by the angle of attack
        angle=alpha*dtor

        xdum2=xdum*cos(angle) - ydum*sin(angle)
        ydum2=ydum*cos(angle) + xdum*sin(angle)

        !equation for ellipse
        r = ((xdum2-x0)/a)**2 + ((ydum2-y0)/b)**2

        if (r .ge. 1) then
            ellipse=0
        else
            ellipse=1
        endif

    end function

    integer function aerofoil(xdum,ydum)
    !given input values of x and y returns 0 or 1 depending upon whether the input
    !coordinartes are within or outwith the aerofoil
    !The aerofoil is taken to be a 4-digit NACA aerofoil (https://en.wikipedia.org/wiki/NACA_airfoil)

        implicit none

        real(8) :: xdum, ydum
        real(8) :: xx, xc
        real(8) :: yc, theta, yt
        real(8) :: xu, yu, xl, yl

        real(8) :: xdum2, ydum2
        real(8) :: angle

        !rotate input coordinates by angle of attack
        angle=alpha*dtor

        xdum2=xdum*cos(angle) - ydum*sin(angle)
        ydum2=ydum*cos(angle) + xdum*sin(angle)

        !shift x coords back so the centre of the aerofoil is at the origin
        xx = xdum2+(c/2.)
        xc=xx/c

        if (xx .lt. 0. .or. xx .gt. c) then
            aerofoil=0
        else
            !create yc
            if (xx .le. p*c) then
                !yc=m*xc/p/p * (2*p-xc) !technically correct, but looks weird for large m and c
                yc=m*xx/p/p * (2*p-xc)
                theta = 2.*m/p/p * (p-xc)
            else
                !yc=m*(c-xc)/(1.-p)/(1.-p) * (1+xc-2*p) !technically correct, but looks weird
                yc=m*(c-xx)/(1.-p)/(1.-p) * (1+xc-2*p)
                theta=2.*m/(1.-p)/(1.-p) * (p-xc)
            endif
            theta=atan(theta)

            yt=5*t *(0.2969*sqrt(xc)-0.1260*xc-0.3516*xc*xc+0.2843*xc*xc*xc-0.1015*xc*xc*xc*xc)

            xu=xx-yt*sin(theta)
            yu=yc+yt*cos(theta)

            xl=xx+yt*sin(theta)
            yl=yc-yt*cos(theta)

            if (ydum2 .lt. yu .and. ydum2 .gt. yl) then
                aerofoil=1
            else
                aerofoil=0
            endif
        endif
    end function


end module
