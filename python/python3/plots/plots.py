...
ax3.plot(
    np.roots(C), np.zeros(len(np.roots(C))), "ro", label="Nullstellen"
)  # Nullstellen
...
xerr = [0.6, 0.7, 0.5, 0.8, 0.7]
yerr = [0.1, 0.2, 0.1, 0.3, 0.2]

ax5.errorbar(x, y, yerr=yerr, xerr=xerr, fmt="r.", label="name1")
...
yerr_upper = [0.2, 0.2, 0.3, 0.2, 0.1]
yerr_lower = np.linspace(0.1, 1, 5)
