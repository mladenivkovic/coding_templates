...
/* =============================================== */
  ...
  int npart_local_start[size];  /* cumulative sum of local number of particles, i.e. starting index of particles for each rank */
  ...
  /* buffer arrays */
  double* x_buf; double* y_buf; double* z_buf; double* m_buf;
  double* fx_buf; double* fy_buf; double* fz_buf;
  double* fx_rbuf; double* fy_rbuf; double* fz_rbuf;
  ...
  for (int loop=0; loop < nloops; loop++){
    ...
    for (int r = 0; r < size; r++){
      if (r==rank){
        ...
        MPI_Send(x_loc, npart_local[rank], MPI_DOUBLE, send_to, 661, MPI_COMM_WORLD);
        MPI_Send(y_loc, npart_local[rank], MPI_DOUBLE, send_to, 662, MPI_COMM_WORLD);
        MPI_Send(z_loc, npart_local[rank], MPI_DOUBLE, send_to, 663, MPI_COMM_WORLD);
        MPI_Send(m_loc, npart_local[rank], MPI_DOUBLE, send_to, 664, MPI_COMM_WORLD);
      }
      /* IMPORTANT!!!! else if, not else! otherwise, you wait for size-1 times to receive something ! */
      else if (r==recv_from){
        /* printf("rank %d receiving from %d\n", rank, recv_from); */
        MPI_Recv(x_buf, npart_local[recv_from], MPI_DOUBLE, recv_from, 661, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(y_buf, npart_local[recv_from], MPI_DOUBLE, recv_from, 662, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(z_buf, npart_local[recv_from], MPI_DOUBLE, recv_from, 663, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(m_buf, npart_local[recv_from], MPI_DOUBLE, recv_from, 664, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
    }


    /* printf("rank %d received %.1lf %.1lf ... %.1lf from rank %d\n", */
    /*     rank, x_buf[0], x_buf[1], x_buf[npart_local[recv_from]-1], recv_from); */
    ...
    /* allocate forces receive buffer so you can receive what happened on other tasks */
    ...
    for (int r = 0; r < size; r++){
      if (r==recv_from){
        /* printf("rank %d wants to send %d to %d\n", rank, npart_local[recv_from], recv_from); */
        MPI_Send(fx_buf, npart_local[recv_from], MPI_DOUBLE, recv_from, 111, MPI_COMM_WORLD);
        MPI_Send(fy_buf, npart_local[recv_from], MPI_DOUBLE, recv_from, 112, MPI_COMM_WORLD);
        MPI_Send(fz_buf, npart_local[recv_from], MPI_DOUBLE, recv_from, 113, MPI_COMM_WORLD);
      }
      if (r==rank){
        /* printf("rank %d wants to receive %d from %d\n", rank, npart_local[rank], send_to); */
        MPI_Recv(fx_rbuf, npart_local[rank], MPI_DOUBLE, send_to, 111, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(fy_rbuf, npart_local[rank], MPI_DOUBLE, send_to, 112, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(fz_rbuf, npart_local[rank], MPI_DOUBLE, send_to, 113, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      }
    }
    ...
    /* deallocate temporary buffer arrays so they can be reallocated in the next loop */
    free(x_buf); free(y_buf); free(z_buf); free(m_buf);
    free(fx_buf); free(fy_buf); free(fz_buf);
    free(fx_rbuf); free(fy_rbuf); free(fz_rbuf);
