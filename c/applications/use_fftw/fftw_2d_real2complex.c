...
int main(void) {
  ...
  int nsamples = 200;
  fftw_complex *out;  // pointers fo type fftw_complex; will contain input and
                      // output of the FFT IMPORTANT: allocate memory with
                      // fftw_malloc instead of malloc! complex type: typedef
                      // double fftw_complex[2]; [0] is real part, [1] is
                      // imaginary part
  ...
  //-------------------------------------
  in = fftw_malloc(sizeof(double) * Nx * Ny);
  out = fftw_malloc(sizeof(fftw_complex) * Nx * Ny);

  //-------------------------------------
  // Create Plan
  //-------------------------------------
  my_plan = fftw_plan_dft_r2c(2, n, in, out, FFTW_ESTIMATE);

  // get plan:
  // rank (dimension) 2
  // n : array containing array lengths per dimension
  // N elements in in and out at first index; N elements in in/out at second
  // index in/out: input/output arrays FFTW_FORWARD: Do Forward transform. for
  // backwards, use FFTW_BACKWARDS. FFTW_ESTIMATE: how well fft must be
  // optimized. i
  ...
  Pk_field = malloc(sizeof(double) * Nx * (Ny / 2 + 1));
  for (i = 0; i < Nx; i++) {
    for (j = 0; j < (Ny / 2 + 1); j++) {
      ind = i * (Ny / 2 + 1) + j;
  ...
  printf("Finished! Written results to ./fftw_output_2d_real.txt\n");
