...
// Usually, this goes into a config.h.
// Move it there in a real project.
...
public:
   ...
   * @param function The current function. Intended to be the __FUNCTION__
   * macro.
   * @param line The current line in the file. Intended to be the __LINE__
   * macro.
   ...
   */
  static void logMessage(const char *file, const char *function, const int line,
                         std::string text, LogLevel level, LogStage stage);

  static void logMessage(const char *file, const char *function, const int line,
                         std::stringstream &text, LogLevel level,
                         LogStage stage);

  static void logMessage(const char *file, const char *function, const int line,
                         const char *text, LogLevel level, LogStage stage);
   ...
   * @param text The message you want to print out.
   */
  static void logWarning(const char *file, const char *function, const int line,
   ...
   * @param text The message you want to print out.
   */
  static void logError(const char *file, const char *function, const int line,
                       std::string text);
  static void logError(const char *file, const char *function, const int line,
...
#define MESSAGE_3_ARGS(msg, level, stage)                                      \
  logging::Log::logMessage(FILENAME_, __FUNCTION__, __LINE__, msg, level,      \
                           stage);

#define MESSAGE_2_ARGS(msg, stage)                                             \
  logging::Log::logMessage(FILENAME_, __FUNCTION__, __LINE__, msg,             \
                           logging::LogLevel::Undefined, stage);
...
#define MESSAGE_STRING_MACRO_CHOOSER(...)                                      \
  MESSAGE_GET_4TH_ARG(__VA_ARGS__, MESSAGE_3_ARGS, MESSAGE_2_ARGS,             \
                      MESSAGE_1_ARG, )
