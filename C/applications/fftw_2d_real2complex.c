...
int main(void)
{
  ...
  // N elements in in and out at first index; N elements in in/out at second index
  ...
  for (i=0; i<Nx; i++){
    for (j=0; j<Ny; j++){
      ind = i*Ny + j;
      in[ind] = cos(2.0*pi/lambda1*i*dx) + sin(2.0*pi/lambda2*j*dy);
    }
  }
  ...
  free(Pk); free(Pk_field); free(distances_k);
