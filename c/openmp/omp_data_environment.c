...
#pragma omp critical /* only one thread at a time */
    {
      printf("Thread %2d has shared: %5d private: %5d firstprivate: %5d "  
...
#pragma omp for lastprivate(lstprvt)
    for (int i = 0; i < 101; i++){
      lstprvt = i + id; /* takes the last value that was assigned to it and passes it as a shared global variable.*/
    }
  ...
  printf("After parallel region:\n");
      printf("              shared: %5d private: %5d firstprivate: %5d " 
        "lastprivate: %5d threadprivate: %5d\n", 
