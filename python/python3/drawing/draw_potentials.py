...
def phiA(x):
    return -14.0 * np.exp(-(x - 12.0) ** 2 / 40) - 0.75
...
def xofPhiA(phi, sign):
    ...
    return 12 + sign * np.sqrt(-40.0 * np.log((phi + 0.75) / -14.0))


def phiB(x):
    return -10.0 * np.exp(-(x - 2) ** 2 / 20) - 0.5
...
def xofPhiB(phi, sign):
    ...
    return 2 + sign * np.sqrt(-20.0 * np.log((phi + 0.5) / -10.0))
...
phiB_part = -8.6
ax.plot(
    [xofPhiB(phiB_part, -1), xofPhiB(phiB_part, 1)], [phiB_part, phiB_part], "b", lw=1
)
phiA_part = -11.6
ax.plot(
    [xofPhiA(phiA_part, -1), xofPhiA(phiA_part, 1)], [phiA_part, phiA_part], "r", lw=1
)
phiC_part = -2.8
ax.plot(
    [xofPhiB(phiC_part, -1), xofPhiA(phiC_part, 1)],
    [phiC_part, phiC_part],
    "forestgreen",
    lw=1,
)
...
ax.plot(
    [xmin * 1.25, xintersect],
    [phiA(xintersect), phiA(xintersect)],
    "k--",
    linewidth=0.5,
)
...
ax.annotate(r"Clump $B$", [2.0, -15.5], color="b", va="top", ha="center")
