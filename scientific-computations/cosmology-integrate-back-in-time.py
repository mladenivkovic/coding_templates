...
#==============================
    ...
    def dadtau(axp_tau):
        dadtau = axp_tau**3 * (omega_m + omega_b + omega_l*axp_tau**3 + omega_k * axp_tau)
...
#=========================================
    ...
    def printline(header):
        for char in header: print("-", end=''); 
        print()
        return

    header = "| {0:10s} | {1:10s} | {2:20s} | {3:20s} | {4:20s} |".format("a",
        "redshift", "lookback time [Gyrs]", "H [km /s /Mpc]", "rho_c [M_Sol/Mpc^3]")
    ...
    for i in range(a_exp.shape[0]):
        print("| {0:10.3f} | {1:10.3f} | {2:20.3f} | {3:20.3f} | {4:20.3e} |".format(
