...
public:
  ...
  void set_position(double x, double y) { _x = x; _y = y;}
private:
  double _x, _y;
...
void struct_with_public_private_vars::public_function(){
  ...
  std::cout << "--- public function called: x=" << _x << " y=" << _y << std::endl;
...
void struct_with_public_private_vars::private_function(){
  // access public members of struct
  std::cout << "------ private function called: i=" << i << " j=" << j << std::endl;
  // access private members of struct
  std::cout << "------ private function called: x=" << _x << " y=" << _y << std::endl;
}
...
int main(){
  ...
  // swppv.x = 3.141; // This throws an error and doesn't compile because x is private.
  swppv.public_function();
  // swppv.private_function(); // This throws an error and doesn't compile because x is private.
