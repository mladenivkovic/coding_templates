...
mutex(void){
  ...
  /*Contains:    */
  /*=============*/
  /*- no mutual exclusion (repeatedly execute to see that sometimes the result is wrong)*/
  /*- critical*/
  /*- atomic*/
...
#pragma omp master
  ...
  /* give threads except 2 some work to "force" it to do the single instruction*/
  int useless = 0;
  if (id != 2){
    for (int i = 0; i==100; i++){
      useless += 1;
...
/*===========================================================================*/
  ...
  int numbers[20] = {5,3,6,7,3, 0,4,2,6,7, 1,1,0,5,3, 2,6,5,3,9};
  ...
  omp_lock_t writelock; /* name your lock. You can have multiple locks with various names. */
