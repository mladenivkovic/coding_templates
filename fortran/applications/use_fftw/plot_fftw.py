...
file_dict["9"] = (
    "fftw_output_convert_convention_complex_fft.txt",
    "1d converting between conventions, Fourier space",
)
file_dict["10"] = (
    "fftw_output_convert_convention_complex_real.txt",
    "1d converting between conventions, real space",
)
file_dict["11"] = (
    "fftw_output_convert_convention_real_fft.txt",
    "1d converting between conventions, Fourier space",
)
file_dict["12"] = (
    "fftw_output_convert_convention_real_real.txt",
    "1d converting between conventions, real space",
)
...
if case in ["6", "8"]:
    ...
    ax.plot(
        x,
        np.cos(d / lambda1) + np.sin(d / lambda2) + np.cos(d / lambda3),
        ":",
        label="expected wave",
    )
    ...
    ax.semilogx(
        k[k > 0] / 2 / np.pi, Pk[k > 0], label="power spectrum"
    )  # ignore negative k
    ...
    if case in ["5", "7"]:
        ax.plot(
            [1 / lambda3] * 2, [Pk.min() - 1, Pk.max() + 1], label="expected lambda3"
        )
